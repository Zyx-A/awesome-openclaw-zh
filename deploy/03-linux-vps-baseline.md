...
### 3. 如果我后面要接企业微信机器人，还能不能继续用 SSH 隧道？

可以，但要分清两件事：

- 第 4 步里的 SSH 隧道，是给你自己在本地浏览器访问 Gateway / UI 用的
- 企业微信机器人接入，是渠道关联问题

按 [`channels/01-wecom-intelligent-bot.md`](../channels/01-wecom-intelligent-bot.md) 这篇文档的主线，企业微信机器人可以走：

- `API 模式`
- `长连接`
- 终端里执行 `openclaw channels add`
- 选择 `Pairing` 完成配对

这条主线同时适用于本地部署和云服务商部署，并没有要求你先把 `18789` 直接暴露到公网。

也就是说，如果你在 Linux VPS 上准备接企业微信机器人，默认仍然可以保持：

- Gateway 只监听 `127.0.0.1:18789`
- 你自己通过 SSH 隧道访问 UI
- 机器人关联在 VPS 终端里完成

只有当你后面明确改走企微文档里提到的 `URL 回调` 方案时，才需要按那条方案单独处理公网入口问题。那已经不是本文默认推荐的“SSH 隧道访问 UI”这条主线。
